...
def main():
    ...
    def compute_metrics(p: EvalPrediction):
        return metric._compute(predictions=p.predictions, references=p.label_ids)
    ...
    def post_processing_function(
        ...
        for idx in range(len(examples)):
            # print(examples[idx].keys())
            output_predict.append({"id": examples[idx]["id"], "prediction_text": formatted_predictions[idx]["prediction_text"],"answers":examples[idx]["answers"],"question": examples[idx]["question"]})
        output_prediction_file = os.path.join(
            training_args.output_dir, "predictions.json")
        with jsonlines.open(output_prediction_file, 'w') as writer:
            for line in output_predict:
                writer.write(line)
